...
//  Note
//
//  Created by Сергей Гриневич on 06/07/2019.
...
class ViewController: UIViewController , UINavigationControllerDelegate{
    ...
    override func viewDidLoad() {
        ...
        noteTextView.text = contex
        bindToKeyboard()
        configGesture()
        navigationController?.delegate = self
        
        navigationItem.leftBarButtonItem = UIBarButtonItem(title: "< Заметки", style: .plain, target: self, action: #selector(backAction))
    ...
    @objc func backAction(){
        ...
        let tableController = navigationController!.viewControllers.last(where: { $0 is TableViewController }) as! TableViewController
        
        if tableController.index == -1 {
            tableController.notes.append(Note(title: titleTextField.text ?? "Text", contex: noteTextView.text ?? "Notes", color: .red, importance: .important))
        } else {
            tableController.notes[tableController.index] =  Note(title: titleTextField.text ?? "Text", contex: noteTextView.text ?? "Notes", color: .red, importance: .important)
        }
